...
function roundDown(a) { return Math.floor(a) }
...
// raiseToPower()
function raiseToPower(m, n) { return m ** n }
...
function getCharCode(a) {
  return `The ASCII code for ${a} is ${a.charCodeAt()}`
}
...
function checkBatteryLevel(percent) {
  if (percent === 100) {
    return "Fully charged :)"
  } else if (percent <= 5) {
    return `Warning - battery level low: ${percent}%, please charge your device`
  } else {
